...
@NamedQueries({
    ...
    @NamedQuery(name = "Booking.sumsByPspAndDayForPersonAndTimePeriod", query = "SELECT t.id,b.day,t.psp,t.name,t.salesRepresentative,t.type,SUM(b.minutes) FROM Booking b,BookingTemplate t WHERE b.bookingTemplateId=t.id AND b.person=:person AND b.day>=:from AND b.day<:to GROUP BY t.id,b.day ORDER BY t.id,b.day"),
